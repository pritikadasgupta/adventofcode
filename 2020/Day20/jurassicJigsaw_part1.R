...
for(i in 1:length(mydata)){
  if(i %in% tile_idx){
    ...
    curdata <- paste(curdata,collapse="")
...
for(b in 1:(length(tile_numbers))){
  ...
  df_tile_numbers <- append(df_tile_numbers,tile_numbers[b])
  top <- paste(cur_tile[10,],collapse="")
  df_tile_position <- append(df_tile_position,"top_rotated")
  df_tile_position_v <- append(df_tile_position_v,top)
  
  
  df_tile_numbers <- append(df_tile_numbers,tile_numbers[b])
  bottom <- paste(cur_tile[1,],collapse="")
  df_tile_position <- append(df_tile_position,"bottom_rotated")
  df_tile_position_v <- append(df_tile_position_v,bottom)
  
  
  df_tile_numbers <- append(df_tile_numbers,tile_numbers[b])
  left <- paste(cur_tile[,10],collapse="")
  df_tile_position <- append(df_tile_position,"left_rotated")
  df_tile_position_v <- append(df_tile_position_v,left)
  
  
  df_tile_numbers <- append(df_tile_numbers,tile_numbers[b])
  right <- paste(cur_tile[,1],collapse="")
  df_tile_position <- append(df_tile_position,"right_rotated")
  df_tile_position_v <- append(df_tile_position_v,right)
...
my_edges_df$value <- as.character(my_edges_df$value)

jigsaw1 <- inner_join(filter(my_edges_df, tile_id != first(tile_id),
                             !grepl('rev', name)), my_edges_df, by = 'value')
jigsaw2 <- filter(jigsaw1,tile_id.x != tile_id.y)
jigsaw3 <- add_count(jigsaw2,tile_id.x, name = 'n.x')
jigsaw4 <- add_count(jigsaw3,tile_id.y, name = 'n.y')
my_jigsaw_df <- jigsaw4

corner <- union(my_jigsaw_df$df_tile_numbers.x[my_jigsaw_df$n.x == 2 & my_jigsaw_df$n.y == 3],
                my_jigsaw_df$df_tile_numbers.y[my_jigsaw_df$n.y == 2 & my_jigsaw_df$n.x == 3])
